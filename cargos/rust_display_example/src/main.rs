use std::fmt; 

// A structure holding two numbers. 'Debug' will be derives so the results can
//  be contrasted with 'Display'
#[derive(Debug)]
struct MinMax(i64, i64);

// Implement 'Display' for 'MinMax'.
impl fmt::Display fro MinMax {
    fn fmy(&self, f: &mut fmt::Formatter) -> fmt::Result {
        // use self.number to refer to each positional data point
        write(f, "{}, {}", self.0, self.1)
    }
}

// Define a structure where the fields are nameable for comparision
#[derive(Debug)]
struct Point2D {
    x: f64,
    y: f64,
}

// Similary, implement 'Disaplay' for 'Point2d'
impl fmt::Display for Point2D {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        // Customize so only 'x' and 'y' are denoted.
        write(f, "x: {}, y: {}", self.x, self.y)
    }
}

fn main() {
    let minmax = MinMax(0, 14);

    println!("Compare structures: ");
    println!("Disaply: {}", minmax);
    println!("Debug: {:?}", minmax); 

    let big_range = MinMax(-300, 300);
    let small_range = MinMax(-3, 3);

    println!("The big range is {big} and the small is {small}", small = small_range, big = big_range);

    let point = Point2D {x: 3.3, y: 7.2};

    println!("Compare points");
    println!("Display: {}", point);
    println!("Debug: {:?}", point);

    // The following line would not vompile: both 'Debug' and 'Display' 
    // were implementedm but '{:b}' requires 'fmt::binary' to be 
    // implemented, which it hasn not been for 'Point2D'. 
    // println!("What does Print2D look like in binary: {:b}?", point)
}
